...
The lexical matcher places the correct item in the top three candidates for every case, but ranks it first in only one in five. Of the 16 failures, 11 are synonym or paraphrase cases and 5 are hard negatives. `syn-rebar` puts `Reinforcement steel` second to `Rebar fabrication and fixing` because the two share almost no tokens, while the two are the same work item to anyone who reads construction descriptions. The hard negatives fail the same way from the other direction: `hard-negative-tiling` prefers `Tiling to wet area walls` over `Tiling to wet area floors` when the query is `Tiling to wet areas`, because the distinguishing token is swamped by the shared one. Both are real failure modes a domain user would report, not metric artifacts.
